document.addEventListener('DOMContentLoaded', () => {
	...
	for (var i = 0, len = squares.length; i < len; i++) {
		...
		(function(index) {
			...
			squares[i].onclick = function(){
				//if the square below this one has class 'taken', you can go here
		...
		function checkBoard() {
			...
			//take the 4 values in each winning array and plug them into the squares values
			...
			for(let y=0; y < winningArrays.length; y++) {
				...
				//check each array to see if all have class 'player-one'
				...
				//check each array to see if al lhave calss 'player-two'
	...
	//event listener to each square to trigger checkBoard
